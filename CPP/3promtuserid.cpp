...
int main(int argc, char *argv[]) {
  ...
  if (file.is_open()) {
    ...
    if (newFile.is_open()) {
      newFile << "This is the initial content of userconfig.txt." << std::endl;
      newFile.close();
      std::cout << "File 'userconfig.txt' has been created." << std::endl;
    } else {
      std::cerr << "Failed to create 'userconfig.txt'." << std::endl;
    }
  }
  ...
  SDL_Window *window = SDL_CreateWindow(
      "Tonk | Master", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 640,
      480, // Window size (width x height)
      SDL_WINDOW_SHOWN);
